<template>
  <div>
      ...
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item label="部门">
            <el-select clearable placeholder="请选择" v-model="maskLayerForm.section_id">
    ...
    >
      ...
      <el-table-column width="200" prop="effectiveTime" label="生效时间" align="center">
        <template slot-scope="scope">
          <!-- <span v-if="scope.row.effectiveTime">{{scope.row.effectiveTime | fmtDate}}</span> -->
          <el-date-picker
          style="width: 180px"
            type="datetime"
      ...
      <el-table-column width="200" prop="ineffectiveTime" label="失效时间" align="center">
        <template slot-scope="scope">
          <!-- <span v-if="scope.row.ineffectiveTime">{{scope.row.ineffectiveTime | fmtDate}}</span> -->
          <el-date-picker
          style="width: 180px"
            type="datetime"
            prefix-icon="el-icon-date"
            value-format="yyyy-MM-dd HH:mm:ss"
            placeholder="失效时间"
            v-model="scope.row.ineffectiveTime"
          ></el-date-picker>
        </template>
      </el-table-column>
    </el-table>
    <div slot="footer" class="dialog-footer text-right" style="margin-top: 15px;">
...
<script>
  ...
  import { attAlreadyBind, attendanceBind } from '@/api/attendance'
  ...
  export default {
    ...
    methods: {
      ...
      async getTableData() {
        const { name, phone, section_id } = this.maskLayerForm
        const { data: { code, msg, data: { list, total } } } = await personnelList({name, phone, section_id, pageSize: 9999, pageNum: 1})
        if (code === '0000') {
          for (const i of list) {
            i['effectiveTime'] = ''
            i['ineffectiveTime'] = ''
      ...
      async getAttAlreadyBind () {
        ...
        if (code === '0000') {
          ...
          for (const i of this.maskTableData) {
            for (const y of data) {
              if (y.id === i.id) {
                if (y.effectiveTime) i['effectiveTime'] = moment(y.effectiveTime).format('YYYY-MM-DD HH:mm:ss')
                if (y.ineffectiveTime) i['ineffectiveTime'] = moment(y.ineffectiveTime).format('YYYY-MM-DD HH:mm:ss')
              }
            }
          }
          // 拿到数据后 反选
      ...
      handleSelectionChange(val) {
        ...
        //     ineffectiveTime: i.ineffectiveTime
        //   })
        // }
      },
      toggleSelection() {
      ...
      async submitForm () {
        ...
        const { data: { code, msg } } = await attendanceBind({
          ruleId: this.rule_id,
          personnelSet: JSON.stringify(formArr)
        })
        if (code === '0000') {
