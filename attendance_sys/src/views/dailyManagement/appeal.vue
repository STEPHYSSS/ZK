<template>
  <el-card id="appeal">
        <div>
            <el-form :model="form" ref="form" :inline="true" size="mini" class="appealForm">
                <el-row>
                    <el-col :span="6">
        ...
        <el-table :data="tableData" class="tableStyle" :header-cell-style="{color:'#444', width: '100%', borderTop: '1px solid #E6E6E6'}">
              ...
              <el-table-column prop="type" label="申诉类型" align="center">
                 <template slot-scope="scope">
                    <span v-if="scope.row.type == 'A'">考勤</span>
                    <span v-if="scope.row.type == 'G'">评分</span>
                    <span v-if="scope.row.type == 'O'">其他</span>
                  </template>
              </el-table-column>
...
<script>
import { appealList } from '@/api/dailyManage'
export default {
    data() {
        return {
    ...
    methods:{
        ...
        async getTableData(){
            ...
            const { data: { code, msg, data: { list, total } } } = await appealList({code: this.form.code, personnelName, startCreateTime, endCreateTime, status, pageSize: this.pageSize, pageNum: this.pageNum})
            if (code === '0000') {
                this.tableData = list
                this.total = total
            }
        ...
        examine(row){
            let roleResult = {}
            if (row.status != 'w') {
                roleResult = this.role.find(item => item.id == 34)
            }else {
                roleResult = this.role.find(item => item.id == 35)
            }
