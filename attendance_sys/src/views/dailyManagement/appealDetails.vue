<template>
    <el-card id="attendanceRecord">
        <div style="padding: 0 20px;">
            ...
            <div class="splitDown">
                <span>申诉类型：</span>
                <i v-if="tableData.type == 'A'">考勤</i>
                <i v-if="tableData.type == 'G'">评分</i>
                <i v-if="tableData.type == 'O'">其他</i>
            </div>
            <div class="splitDown">
                <span>申诉描述：</span>{{tableData.remark}}
            </div>
            <div class="splitDown">
                <span>附件：</span>
...
<script>
import { appealDetail, appealCheck } from '@/api/dailyManage'
export default {
    data(){
        return{
            form: {
    ...
    methods:{
        async getAppealDetail () {
            const { data: { code, data } } = await appealDetail({code: this.code})
            if (code === '0000') {
                this.tableData = data
