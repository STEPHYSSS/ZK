  ...
  <div class="shadow-card">
    ...
    <el-form ref="form" :inline="true" :model="form" size="mini" style="padding: 20px 20px 0 20px;" class="form-query demo-form-inline">
      <el-row>
        ...
        <el-col :span="6">
          <el-form-item label="部门:" prop="section_id">
            <el-select clearable v-model="form.section_id" placeholder="请选择">
        ...
        <el-col class="fr text-right" :span="6">
          <el-form-item>
            <el-button class="border-blue bgc-blue font-color-fff" @click="getPersonnelList">查询</el-button>
    ...
    <el-button size="mini" class="add-btn font-size-12 border-orange bgc-orange font-color-fff" icon="el-icon-plus" @click="addUser">新增</el-button>
    ...
    <el-table :data="tableData" :header-cell-style="{color:'#444', width: '100%', borderTop: '1px solid #E6E6E6'}">
      ...
      <template slot-scope="scope">
        <i class="font-color-orange cur-point" @click="editUser(scope.row.id)">编辑</i>
...
import { personnelList, personnelrePwd } from '@/api/user'
...
export default {
  ...
  created () {
    if (sessionStorage.role) this.role = JSON.parse(sessionStorage.role)
  ...
  methods: {
    ...
    async getPersonnelList () {
      ...
      const { data: { code, msg, data: { total, list } } } = await personnelList({ name, phone, station, status, pageSize, pageNum })
      if (code === '0000') {
    ...
    //addUser
    addUser () {
      if (!this.role) this.role = JSON.parse(sessionStorage.role)
      const roleResult = this.role.find(item => item.id == 12)
    ...
    editUser (id) {
      if (!this.role) this.role = JSON.parse(sessionStorage.role)
      const roleResult = this.role.find(item => item.id == 13)
      if (roleResult.check == 1) {
        this.$router.push({
          name: 'addUser',
          query: { id }
    ...
    resetPwd (id) {
      if (!this.role) this.role = JSON.parse(sessionStorage.role)
      const roleResult = this.role.find(item => item.id == 14)
      if (roleResult.check == 1) {
        this.rePwdId = id
    ...
    // 确认重置
    async conRePwd () {
      const { data: { code, msg } } = await personnelrePwd({id: this.rePwdId})
      if (code === '0000') {
