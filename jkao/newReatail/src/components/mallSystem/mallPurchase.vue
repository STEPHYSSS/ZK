  ...
  <div class="mallPurchase">
    ...
    <div class="storeInfo">
      <div class="store_Box1">
        <div class="addStore">
          <router-link to="/mallPurchaseAdd">
            ...
            <template scope="scope">
              <el-tooltip class="item" effect="dark" content="查看" placement="bottom">
                <img
                  ...
                  @click="malldetail(scope.row.code)"
                  class="imgSize"
                  />
              </el-tooltip>
              <el-tooltip
...
export default {
  ...
  methods: {
    ...
    getOurWare(pageNum, pageSize) {
      this.$axios({
        url: window.apiMall + "purchase/list",
        method: "POST",
    ...
    delPurWare(code) {
      this.$axios({
        url: window.apiMall + "/purchase/delete",
        method: "POST",
        headers: { "content-type": "application/x-www-form-urlencoded" },
        data: qs.stringify({
          token: sessionStorage.getItem("token"),
...
.mallPurchase .el-range-editor.el-input__inner {
  width: 100%;
}
.mallPurchase .el-select {
  width: 100%;
}
.storeBox222 {
  height: 150px;
