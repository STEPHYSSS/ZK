<template>
  <div class="operaPurWare">
    ...
    <div class="storeBox storeBox222">
      <div class="store_Box">
        <p class="store_tit">采购入库</p>
        <el-row style="white-space: nowrap;">
          <el-col :span="6">
            <span>创建日期：</span>
            <p>
              <el-date-picker v-model="createTime" @change="changeCreatT" type="daterange" 
    ...
    <div class="storeInfo">
      <div class="store_Box1">
        <div class="addStore">
          <router-link to="/operaPurWareAdd">
        ...
        <el-table :data="tableData" style="width: 100%;">
              <el-table-column prop="code" label="单据编号" align='center' width="250">
              </el-table-column>
              <el-table-column label="创建时间" align='center'>
                <template scope="scope"><span v-if="scope.row.createTime !=null">{{scope.row.createTime | converTime('YYYY-MM-DD')}}</span></template>
              </el-table-column>
              <el-table-column label="入库状态" align='center'>
                  <template scope="scope">{{scope.row.status | statusTip}}</template> 
              </el-table-column>
              <el-table-column prop="item" label="入库品项数" align='center'>
              </el-table-column>
              <el-table-column prop="count" label="入库总数" align='center'>
              </el-table-column>
              ...
              <el-table-column label="操作" align='center' fixed="right">
                  <template scope="scope">
                      <el-tooltip class="item" effect="dark" content="查看" placement="bottom">
                        <img src="@/assets/chaxun_icon.png" @click="purWareDetail(scope.row.code)" class="imgSize">
                      </el-tooltip>
                      <el-tooltip class="item" effect="dark" content="删除" placement="bottom" v-if="scope.row.status == 'F'">
                        <img src="@/assets/shanchu_icon.png" @click="delPurWare(scope.row.code)" class="imgSize">
                      </el-tooltip>
                  </template>
              </el-table-column>
            </el-table>
...
export default {
  ...
  methods: {
    ...
    getOurWare(pageNum, pageSize) {
      this.$axios({
        url: window.apiUrl + "warehouse/purchase/list",
        method: "POST",
    ...
    delPurWare(code) {
      this.$axios({
        url: window.apiUrl + "warehouse/purchase/delete",
        method: "POST",
        headers: { "content-type": "application/x-www-form-urlencoded" },
        data: qs.stringify({
          token: sessionStorage.getItem("token"),
    ...
    // 查看
    purWareDetail(code) {
      sessionStorage.setItem("purWareC", code);
      this.$router.push({
...
.operaPurWare .el-range-editor.el-input__inner {
  width: 100%;
}
.operaPurWare .el-select {
  width: 100%;
}
.storeBox222 {
  height: 150px;
