...
export default {
    ...
    methods: {
        ...
        getCheckInfo() {
            this.$axios({
                url: window.apiUrl + "amendment/detail",
                method: "POST",
                headers: {
                ...
                data: qs.stringify({
                    ...
                    amendmentCode: sessionStorage.getItem("xiuCheckCode")
                })
            }).then(res => {
                const {
                    code,
        ...
        xiuBaocun() {
            ...
            _this.checkTable.forEach((item, index) => {
                if (item.finalRuleCount) {
                    _this.list.push({
                        id: item.id,
                        finalRuleCount: item.finalRuleCount
                    });
                } else {
            ...
            _this
                .$axios({
                    url: window.apiUrl + "amendment/check",
                    method: "POST",
                    headers: {
                        "content-type": "application/x-www-form-urlencoded"
                    },
                    data: qs.stringify({
                        token: sessionStorage.getItem("token"),
                        amendmentCode: sessionStorage.getItem("xiuCheckCode"),
                        headquartersOpinion: _this.title,
                        jsonArray: JSON.stringify(_this.list)
                    })
                })
                .then(res => {
                    if (res.data.code == "0000") {
                        _this.$router.push({ name: "sheet" });
                    }
                });
        },
        back() {
