    ...
    props: ['caseBh'],
		...
		methods: {
			initCourses () { //获取案例与纳税人信息
				...
				that.$utils.post(`tax/useCaseByBh/${that.$route.params.caseBh}`).then(res => {
          that.nsrsbh = res.data.nsrxx.nsrsbh
					that.nsrmc = res.data.nsrxx.nsrmc
					this.now = res.data.sbqx
					this.value1 = res.data.sbqx
					sessionStorage.setItem('useCase', JSON.stringify(res.data));
					sessionStorage.setItem('sjh', res.data.sjh);
					// let t = null;
