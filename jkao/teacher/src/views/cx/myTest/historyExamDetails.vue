<template>
  <div class="exam-box box-card">
    <div class="previousPage" id="previousPage"><img src="@/assets/images/returnbutton.png" @click="goback" /></div>
    <!-- 返回 -->
    <!-- 考试界面身体 -->
    <div class="exam-body">
      <!-- 试题部分 -->
...
export default {
  ...
  created() {
     ...
     */
    this.detailHistory(this.$route.query.exam_id, this.$route.query.testDetail); // 获取考试信息
  },
  components: {
    examDetail,
    examTabs,
  ...
  methods: {
    ...
    async detailHistory(exam_id, testDetail) {
      ...
      if (testDetail == 1) {
        ...
        const { code, msg, singleObjs, multiObjs, judgeObjs, prcQus, records, selfExam } = res.data
        if (code == "0000") {
        ...
        // 调用
        this.getRecord( records, singleObjs, multiObjs, judgeObjs, prcQus );
      }
      ...
      const { code, msg, singleObjs, multiObjs, judgeObjs, prcQus, records, exam_flow } = res.data
      if (code == "0000") {
        // 这三个遍历 手动添加考生所选选中答案字段
        for (const i of singleObjs) { this.$set(i, "checkAnswer", "") }
        for (const i of multiObjs) { this.$set(i, "checkAnswer", "") }
