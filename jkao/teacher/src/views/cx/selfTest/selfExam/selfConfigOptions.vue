<template>
  <el-card style="min-width: 1090px" shadow="never" id="config-select" class="box-card">
    <div class="font-weight-700 student_title">新增自测</div>
    <previous-page goPreviousPage="/selfExamList" class="createBack"></previous-page>
    <!-- <div class="new-padding-left ex"></div> -->
    <el-form ref="userConfigOptions" :model="userConfigOptions" label-width="80px" class="config-big">
      ...
      <div class="config-left ">
        ...
        <div class="config-left-neck txalign">
          <div class="neck-input">
            <span><i style="color:red">*</i>自测名称: <input type="text" v-model="userConfigOptions.self_exam.name" maxlength="20"></span>
            <span>自测总分: <input type="text" v-model="userConfigOptions.self_exam.total_score" :max="9999" :min="1"></span>
            <el-button @click="computeTotal(userConfigOptions.sets)" class="computeTotal">计算总分</el-button>
          </div>
        </div>
        <ul>
            ...
            <draggable id="to-test" class="con-show-quest" v-model="userConfigOptions.sets" :options="optionDrag" @update="update" @add="add($event, userConfigOptions.sets)">
              ...
              <div class="hideDom" style="height: 100%">
                <div class="config-body-footer" v-for="(item, index) in userConfigOptions.sets" :key="index + 'a'">
                  <template v-if="item.type == 1 && item.question_kind == 1">
                    <div class="footer-question-left">分值:
                      <input type="text" maxlength="10" v-model="item.set_score" class="short-input">
                      <span class="unilineText1">题干：{{item.content}}</span>
                    </div>
              ...
              <div class="hideDom" style="height: 100%">
                <div class="config-body-footer" v-for="(item, index) in userConfigOptions.sets" :key="index + 'b'">
                  <template v-if="item.type == 2 && item.question_kind == 1">
                    <div class="footer-question-left">分值:
                      <input type="text" maxlength="10" v-model="item.set_score" class="short-input">
                      <span class="unilineText1">题干：{{item.content}}</span>
                    </div>
              ...
              <div class="hideDom" style="height: 100%">
                <div class="config-body-footer" v-for="(item, index) in userConfigOptions.sets" :key="index + 'c'">
                  <template v-if="item.type == 3 && item.question_kind == 1">
                    <div class="footer-question-left">分值:
                      <input type="text" maxlength="10" v-model="item.set_score" class="short-input">
                      <span class="unilineText1">题干：{{item.content}}</span>
                    </div>
              ...
              <div class="hideDom" style="height: 100%">
                <div class="config-body-footer" v-for="(item, index) in userConfigOptions.sets" :key="index + 'd'">
                  <template v-if="item.question_kind == 2">
                    <div class="footer-question-left">分值:
                      <input type="text" maxlength="10" v-model="item.set_score" class="short-input">
                      <span class="unilineText1">题干：{{item.content}}</span>
                    </div>
      ...
      <div class="config-right">
        <div style="position:fixed;">
          <div class="config-left-header txalign">从题库中筛选试题加入到当前试卷</div>
          <el-form :inline="true" :model="form" class="demo-form-inline">
            <el-form-item>
              <el-select v-model="question_kind" @change="checkKind" placeholder="试题类型" class="searchWid-1">
                <el-option label="客观题" value="1"></el-option>
                <el-option label="实训题" value="2"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item>
              <el-select v-if="question_kind === '1'" clearable @change="questionList(pagesize, pagenum)" v-model="form.type" placeholder="试题类型" class="searchWid-3">
...
<script>
  import { typePaperSet, papersetDetail } from '@/api/taxUrl/testpaper'
  ...
  export default {
    ...
    methods: {
      ...
      //   if (code == '0000') {
      //     that.object = testpaper
      //     for (const i of back) {
      //       i.content = i.content.replace(/<[^>]+>|&[^>]+;/g,"").trim()
      //     }
      //     that.userConfigOptions.sets = back
      //     Object.keys(that.object).forEach(key => {
      //       if (that.userConfigOptions[key] !== undefined) that.userConfigOptions[key] = that.object[key]
      //     })
      ...
      async questionList (pageSize, pageNum) {
        ...
        if (code == '0000') {
          for (const i of questions.list) {
            i.content = i.content.replace(/<[^>]+>|&[^>]+;/g,"").trim()
            if (that.question_kind === '1') {
              that.$set(i, 'question_kind', 1)
            }else {
              that.$set(i, 'question_kind', 2)
            }
          }
      ...
      add (e, sets) {
        // debugger
        let obj = this.userConfigOptions.sets
        if (e.from.id === 'to-test') return false
        if (e.from.id === 'from-test') {
          ...
          this.userConfigOptions.sets = this.userConfigOptions.sets.map((addMap, index) => {
            ...
            return {
              question_uuid,
              question_kind: addMap.question_kind,
              self_exam_id: null,
              order_by: index + 1,
              type: addMap.type,
              set_score,
      ...
      async submitForm() {
        if(!this.userConfigOptions.self_exam.name) return this.$message.error('请填写名称！！')
        const that = this
        if (that.userConfigOptions.sets.length <= 0) return that.$message('请添加试题')
        if (that.userConfigOptions.self_exam.name == "") return that.$message('请输入自测名称')
