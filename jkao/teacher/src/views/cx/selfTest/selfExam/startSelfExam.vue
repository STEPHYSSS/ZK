<template>
    <div class="exam-box">
        <div class="previousPage" style="margin-top: 5px;">
            ...
            <div class="exam-question">
                ...
                <div class="tab-content">
                    ...
                    <button
                        ...
                        id="positionBtn"
                        @click="submitPaper1"
                        v-if="jilurecords.length > 0"
                        :class="{selfExam:this.$route.query.flagBack}"
                    >交 卷</button>
                    <el-dialog title="提示" :visible.sync="dialogVisible" width="30%" height="16%">
...
export default {
    ...
    created() {
        ...
        if (this.$route.query.showQuestion)
            this.showQuestion = this.$route.query.showQuestion - 0;
    ...
    methods: {
        ...
        async selfExamStart() {
            ...
            } = await selfExamStart({ self_exam_id: this.id });
            if (code == "0000") {
                ...
                /* 获取实训题记录 */
                this.getPrcRecord(examFlow, prcQus);
                // 调用
        ...
        async selfExamGetRecord(
            ...
            const {
                data: { code, records }
            } = await selfExamGetRecord({
                self_exam_id: sessionStorage.getItem("exam_id")
            });
        ...
        async getPrcRecord(examFlow, prcQus) {
            const {
                data: { code, data }
            } = await this.$utils.post(this.reqApi.xinls+'/exam/question/finish/get',qs.stringify({examFlow}))
            if (code == "0000" && data.length > 0) {
                for (const i of data) {
                    for (const prc of prcQus) {
                        if (i === prc.practice_question_uuid) {
                            prc.checkAnswer = 1;
                        }
                    }
                }
