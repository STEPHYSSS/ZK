    ...
    <div class="storehouse_box">
      ...
      <div class="block">
        ...
        <div class="hideBtn" v-if="this.AreaCode !='' && this.goodsSet !=''">
          <el-button class="AllquedingBtn" @click="submitForm">确定</el-button>
              ...
              <el-table-column align="center" width="55">
                <template slot-scope="scope">
                  <el-radio-group v-model="supperRow">
                    <el-radio :label="scope.row.code" @change="chooseSupper(scope.row.code)">&nbsp;</el-radio>
                  </el-radio-group>
                </template>
              </el-table-column>
...
export default {
  ...
  methods: {
    ...
    showSupp(pageNum3, pageSize3) {
      ...
      this.$axios
        .post(
          this.reqApi.xinls + `/exam/supplier/list`,
          qs.stringify({
            token: sessionStorage.getItem("token"),
    ...
    submitForm() {
      ...
      if (!goodCounts) {
        ...
        this.$axios
        .post(
          this.reqApi.xinls + `/exam/warehouse/purchase/insert`,
          qs.stringify({
            token: sessionStorage.getItem("token"),
            questionCode: sessionStorage.getItem("questionUUid"),
            warehouseCode: this.regionId,
        ...
        .then(res => {
          if (res.data.code == "0000") {
            ...
            this.$router.push({ name: "CKProcurement" });
          } else if (res.data.code == "6666") {
            this.$message({
              showClose: true,
              message: "token异常，请重新登录",
