        ...
        <div>
            ...
            >
                ...
                <el-table-column label="出库单编号" align="center" width="250">
                    <template slot-scope="scope">
                        <span
                            class="codesty"
                            @click="toDetail(scope.row.code,scope.row)"
                        >{{scope.row.code}}</span>
                    </template>
                </el-table-column>
            ...
            <span class="cur-point dis-inline-block">
              <img src="@/images/return.png" v-if="scope.row.status != 'S' && scope.row.status!='N'" @click="backStatus(scope.row.code)">
                        ...
                        <!-- <span
                            v-if="scope.row.status != 'S' && scope.row.status!='N'"
                            @click="backStatus(scope.row.code)"
                            class="backStatusSty"
                        ></span> -->
                         <el-tooltip class="item" effect="dark" content="回退" placement="bottom">
                            <img
                                src="@/assets/images/fanhui_icon.png"
                                @click="backStatus(scope.row.code)"
                                class="codesty"
                                 v-if="scope.row.status != 'S' && scope.row.status!='N'"
                                alt
        ...
        <span slot="footer" class="dialog-footer">
          <el-button class="theBtn themeColor textMright" @click="sure">确定</el-button>
            ...
            <el-dialog title="提示" :visible.sync="centerDialogVisible1" width="20%" center>
                ...
                <span slot="footer" class="dialog-footer">
                    <el-button
                        class="queryButton increase"
                        @click="createSortingSure"
                        v-model="goOn"
                    >继续拣货</el-button>
                    <el-button
                        class="resetButton btnAdd3 btnAdd2"
    ...
    methods: {
        ...
        delPanRow(code) {
            this.$confirm("删除后员工将无法登陆,确定提交?", "提示:", {
                ...
                this.$utils({
                url: this.reqApi.xinls + "/exam/warehouse/checkout/delete",
                method: "POST",
                headers: {
            ...
            this.$refs[formName].validate(valid => {
                if (valid) {
                    this.$utils
                        .post(
                            this.reqApi.xinls +
                                "/exam/warehouse/checkout/review",
                            qs.stringify({
        ...
        getList() {
            this.$utils
                .post(
                    this.reqApi.xinls + "/exam/warehouse/checkout/list",
                    qs.stringify({
                        questionCode: sessionStorage.getItem("questionUUid")
                    })
                )
