      ...
      <div class="billboxTab">
        <table class="deliveTab">
          <tr>
            ...
            <td class="rowwith">报废单编号：{{list.code}}</td>
            <td class="rowWith">门店：{{list.storeCode}}</td>
            <td class="rowwith">报废状态：{{list.status |statusTip }}</td>
            <!-- (对应的是后台的123代码 1待审核2 审核通过 3 审核不通过) -->
          </tr>
          <tr>
            <td class="rowWith">报废品项数：{{list.item}}</td>
            <td class="rowwith">报废总数：{{list.count}}</td>
            <td class="rowWith">报废总金额：{{list.amount}}</td>
            <td class="rowWith" v-if="list.checkOpinion">审核意见：{{list.checkOpinion}}</td>
          </tr>
          <tr>
            <td></td>
            <td></td>
...
export default {
  ...
  methods: {
    ...
    getlist() {
      this.$utils
        .post(this.reqApi.xinls+"/exam/scrap/detail", qs.stringify({
          questionCode: sessionStorage.getItem("questionUUid"),
          code:this.$route.query.code
        }))
        .then(res => {
    ...
    SubmitAduit(){
        ...
        this.$utils.post(this.reqApi.xinls+'/exam/scrap/check',qs.stringify(this.ruleForm)).then(res=>{
            if(res.data.code==='0000'){
              this.centerDialogVisible=false
              this.$router.push({
                name:'scrapManage'
