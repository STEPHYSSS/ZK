<template>
  <el-card class="f-new box-card">
    ...
    <h3 v-if="this.$route.query.id">修改实训题库</h3>
    <h3 v-else>新增实训题库</h3>
...
export default {
  ...
  methods: {
    getQuestionInfo() {
      this.$utils
        .post(
          this.reqApi.shuiwuUrl + "/question/prc/detailBank",
          qs.stringify({
        ...
        .then(res => {
          if (res.data.code === "0000") {
            this.ruleForm = res.data.prcQuBank;
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          ? this.reqApi.shuiwuUrl + "/question/prc/createBank"
          : this.reqApi.shuiwuUrl + "/question/prc/updateBank";
              ...
              .post(
                ...
                qs.stringify({
                  bank: JSON.stringify({
                    ...
                    practice_bank_id: that.$route.query.id
                  })
                })
              )
              .then(res => {
