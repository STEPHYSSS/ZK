    ...
    <el-card class="f-new box-card">
        ...
        <template slot-scope="scope">
          ...
          <span v-if="scope.row.ppapertype == '0'">普通试卷</span>
            ...
            <el-table-column prop="remark" show-overflow-tooltip align="center" label="试卷说明"></el-table-column>
            ...
            <el-table-column align="center" label="锁卷状态">
                <template slot-scope="scope">
            ...
            <el-table-column align="center" label="操作" fixed="right" width="180">
                <template slot-scope="scope">
                    ...
                    <span v-if="scope.row.locked == 2">
                        |
                        <a href="javacript:;" @click="suojuan(scope.row)">锁卷</a>
                    </span>
                    <span v-if="scope.row.locked == 1">
                        |
                        <a href="javacript:;" @click="suojuan(scope.row)">解锁</a>
                    </span>
                    |
                    ...
                    <el-tooltip class="item" effect="dark" content="组卷" placement="bottom">
                      <span class="cur-point dis-inline-block" @click="handleConfigOptions(scope.row)">
                        <img src="@/assets/images/zujuan_icon.png" alt />
                      </span>
                    </el-tooltip>&nbsp;&nbsp;
                    <el-tooltip v-if="scope.row.locked==2" class="item" effect="dark" content="锁卷" placement="bottom">
                      <span class="cur-point dis-inline-block el-icon-lock" style="font-size: 17px; color: #5691F6;" @click="suojuan(scope.row)"></span>
                  </el-tooltip>
                  <el-tooltip v-if="scope.row.locked==1" class="item" effect="dark" content="解锁" placement="bottom">
                    <i class="el-icon-unlock" style="font-size: 17px; color: #5691F6;"  @click="suojuan(scope.row)"></i>
                  </el-tooltip>&nbsp;&nbsp;
                  <el-tooltip class="item" effect="dark" content="复制" placement="bottom">
                    <span class="cur-point" @click="copyPaper(scope.row)">
...
export default {
    ...
    methods: {
        ...
        copyPaper(data) {
            this.$utils
                .post(
                    this.reqApi.shuiwuUrl + "/testpaper/copy",
                    qs.stringify({
            ...
            if (info.locked == 1) {
                this.$confirm("确认解锁该试卷？", "提示", {
                    ...
                    .then(() => {
                        this.$utils
                            .post(
                                this.reqApi.shuiwuUrl + "/testpaper/unlock",
                                qs.stringify({
        ...
        handleExamDetails(user) {
            this.$utils
                .post(`exam/sys/findExamSetByPid/${user.pid}`)
                .then(res => {
