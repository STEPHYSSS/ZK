<template>
  <div class="question-list">
    <i class="el-icon-d-arrow-right rotate-c cur-point" @click="isShowAll"></i>
    <div class="q-header flex-lr">
      <span class="font-weight-700">
        <span style="margin-right: 35px;">试题类型：判断题</span>
        <span>判断题总分：{{scoreSum}}</span>
      </span>
    ...
    <el-dialog
      ...
      width="1000px">
      <question-dialog v-if="dialogVisible" :isSelf="isSelf" :showQuestion="showQuestion" @addQuestion="addQuestion" :type="3" :kind="'1'" @handleClose="dialogVisible=false"></question-dialog>
    </el-dialog>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      changePoint (data) {
        this.$emit('changePoint', [data, 'judgeList'])
      },
      checkPoint (val, id) {
        let g = val[id] + ''
        g = g.replace(/[^0-9]/g,'')
      ...
      addQuestion (data) {
        ...
        this.$emit('questionSort', [data, 'judgeList'])
        // 计算分值
        this.changePoint(data)
      },
      isShowAll () {
        this.isShow = !this.isShow
        document.querySelector('.rotate-c').style.transform = this.isShow ? "rotateZ(225deg)" : "rotateZ(45deg)"
        // 隐藏
