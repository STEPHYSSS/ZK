<template>
  <div class="question-list">
    <i class="el-icon-d-arrow-right rotate-d cur-point" @click="isShowAll"></i>
    <div class="q-header flex-lr">
      <span class="font-weight-700">
        <span style="margin-right: 35px;">试题类型：实训题</span>
        <span>实训题总分：{{scoreSum}}</span>
      </span>
    ...
    <el-dialog
      ...
      width="1000px">
      <question-dialog v-if="dialogVisible" :prcType="'prcType'" :isSelf="isSelf" :showQuestion="showQuestion" @addQuestion="addQuestion" :type="1" :kind="'2'" @handleClose="dialogVisible=false"></question-dialog>
    </el-dialog>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      changePoint (data) {
        this.$emit('changePoint', [data, 'prcType'])
      },
      checkPoint (val, id) {
        let g = val[id] + ''
        g = g.replace(/[^0-9]/g,'')
      ...
      addQuestion (data) {
        ...
        this.$emit('questionSort', [data, 'prcType'])
        // 计算分值
        this.changePoint(data)
      },
      isShowAll () {
        this.isShow = !this.isShow
        document.querySelector('.rotate-d').style.transform = this.isShow ? "rotateZ(225deg)" : "rotateZ(45deg)"
        // 隐藏
