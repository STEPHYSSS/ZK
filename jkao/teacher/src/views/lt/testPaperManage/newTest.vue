      ...
      <div class="new-body">
        <el-form ref="form" :rules="rules" :model="form" label-width="100px">
          <div>
            ...
            <el-form-item label="开考时间:" prop="start_time" class="wushi fr">
              <el-date-picker
                type="datetime"
          ...
          <div>
            <el-form-item label="考试时长:" prop="duration" class="wushi">
          ...
          <div>
            <el-form-item label="目标班级:" prop="class_ids" class="wushi">
              <el-select v-model="class_ids" multiple placeholder="请选择" class="inputWid">
                <el-option
          ...
          <el-form-item label="考试说明:" prop="description">
              ...
              <p class="txalign-c paper-p">{{form.description.length}}/200 &nbsp;&nbsp;&nbsp;</p>
          </el-form-item>
          <add-btn
...
<script>
  ...
  export default {
    data() {
      ...
      return {
        ...
        rules: {
          ...
          start_time: [{ required: true, message: '开考时间不能为空！', trigger: 'blur' }],
          duration: [ { required: true, message: '考试时长不能为空！', trigger: 'blur' } ],
          end_time: [{ required: true, validator: checkExamTime, trigger: 'blur' }],
          description: [{ required: true, message: '考试说明不能为空！', trigger: 'blur' }]
        }
      }
    },
    ...
    computed: {
      level: {
        get: function () {
          return this.level = !this.$route.query.id ? "新建考试" : "修改考试"
        },
        set: function () {}
      },
      duration () {
        let duration = (this.form.end_time - this.form.start_time) / 1000 / 60
        if (duration >= 0) {
    ...
    methods: {
      ...
      // 获取单个考试信息
      async getOnePaper () {
        const { data: { exam, classes, code } } = await examDetail(this.$route.query.id - 0)
        if (code == '0000') {
          this.form = exam
        ...
        that.$refs[formName].validate(async (valid) => {
          if (valid) {
            ...
            if (!that.$route.query.id) {
              if(this.form.testpaper_id == null) return this.$message("请选择试卷")
