<template>
  <div class="slipInfo">
    ...
    <p class="store_tit">门店盘点</p>
        <router-link :to="{ path: '/operaInventory' }" class="OPBtnBack">
...
export default {
  ...
  methods: {
    getSlipDetail() {
      this.$axios({
        url: window.apiUrl + "inventory/detail",
        method: "POST",
        headers: { "content-type": "application/x-www-form-urlencoded" },
        data: qs.stringify({
    ...
    sureYs(code, status) {
      this.$confirm("验收后数量无法更改, 是否提交?", "提示", {
        ...
        .then(() => {
          this.$axios({
            url: window.apiUrl + "inventory/check",
            method: "POST",
            headers: { "content-type": "application/x-www-form-urlencoded" },
            data: qs.stringify({
              code: code,
            ...
            if (res.data.code == "0000") {
              ...
              this.$router.push({ name: "operaInventory" });
            } else if (res.data.code == "6666") {
              this.$message({
                showClose: true,
                message: "token异常，请重新登录",
