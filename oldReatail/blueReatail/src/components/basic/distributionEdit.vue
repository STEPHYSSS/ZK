  ...
  methods: {
    ...
    submit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$axios
            .post("/warehouse/distribution/update", qs.stringify({
              token:sessionStorage.getItem("token"),
              name:this.ruleForm.name,
              priority:this.ruleForm.priority,
              countRule:this.ruleForm.countRule,
              turnoverRule:this.ruleForm.turnoverRule,
              status:this.ruleForm.status,
              distributionRule:this.ruleForm.distributionRule,
              createRule:this.ruleForm.createRule,
            }))
            .then(res => {
              if (res.data.code === "0000") {
                this.$router.push({
