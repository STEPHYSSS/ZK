  ...
  <div class="Advertising minimum">
    <div class="cbox">
      ...
      <el-row>
        <el-col :span="8">
          <div>
            <span class="textTitle textTitleR textMright">模块名称:</span>
    ...
    <el-table :data="tableData" style="width: 100%" max-height="600" border>
      <el-table-column prop="name" align="center" label="模块名称" :show-overflow-tooltip="true"></el-table-column>
...
export default {
  ...
  methods: {
    getList() {
      this.$axios
        .post("/mall/model/list", qs.stringify(this.dataList))
        .then(res => {
    ...
    del() {
      console.log(this.id)
       this.$axios
        .post("/mall/model/delete", qs.stringify({
          token:sessionStorage.getItem('token'),
          id:this.id
        }))
        .then(res => {
