<template>
    <div class="outBox">
        <div class="outBox_one">
            <van-nav-bar
                :title="$route.query.type?'申诉详情':'新建申诉'"
                left-arrow
                fixed
            ...
            <div>
                <van-cell title="申诉类型" is-link @click="show=true">
                    <template #default>
                        <span style="color:#14C25AFF">{{form.type|filterValue}}</span>
                    </template>
                </van-cell>
            ...
            <!-- 拿到详情页的时候进行展示如下部分 -->
            <div class="imgBox" v-if="$route.query.type">
                <van-cell title="提交人" :value="form.personnelName" />
                <van-cell title="提交时间" :value="form.createTime" />
                <van-cell title="状态" value="待审核" />
                <div v-if="$route.query.active==2">
                    <van-cell title="审核">
                        <template #default>
                  ...
                  <div v-if="$route.query.status=='S'||$route.query.status=='F'">
                    <van-cell title="审核">
                        <template #default>
                            <!-- <el-radio v-model="isPass" label="1"></el-radio>
                            <el-radio v-model="isPass" label="2"></el-radio> -->
...
export default {
    ...
    created() {
        this.getDetail();
        if (this.$route.query.status == "W"||this.$route.query.status == "F"||this.$route.query.status == "S") {
            this.upload = false;
        }
    },
    mounted() {
        ...
        // if (this.$route.query.status == "W" || this.manager == "1") {
    ...
    methods: {
        ...
        submit() {
            ...
            if(this.$route.query.active==2){
                this.$utils.post(this.reqApi.yiBSchool+'/mobile/appeal/check',qs.stringify({
                    code:this.form.code,
